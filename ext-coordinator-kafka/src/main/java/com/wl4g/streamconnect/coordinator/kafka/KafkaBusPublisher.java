...
import com.wl4g.streamconnect.coordinator.IStreamConnectCoordinator.BusEvent;
import com.wl4g.streamconnect.coordinator.IStreamConnectCoordinator.IBusPublisher;
import com.wl4g.streamconnect.coordinator.kafka.KafkaStreamConnectCoordinator.KafkaCoordinatorConfig;
import com.wl4g.streamconnect.coordinator.kafka.KafkaStreamConnectCoordinator.KafkaCoordinatorProvider;
...
public class KafkaBusPublisher implements IBusPublisher {
    ...
    public KafkaBusPublisher(StreamConnectConfiguration config) {
        ...
        if (isNull(busProducer)) {
            synchronized (this) {
                if (isNull(busProducer)) {
                    ...
                    config.putAll(KafkaStreamConnectCoordinator.DEFAULT_PRODUCER_PROPS);
