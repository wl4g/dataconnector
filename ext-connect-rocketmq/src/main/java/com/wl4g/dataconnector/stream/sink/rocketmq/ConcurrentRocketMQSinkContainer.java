...
package com.wl4g.dataconnector.stream.sink.rocketmq;
...
public class ConcurrentRocketMQSinkContainer implements Closeable {
    private final RocketMQSinkStream.RocketMQSinkStreamConfig sinkStreamConfig;
    private final ChannelInfo channel;
    private final List<DefaultMQProducer> producers;

    public ConcurrentRocketMQSinkContainer(RocketMQSinkStream.RocketMQSinkStreamConfig sinkStreamConfig,
                                           ChannelInfo channel) {
    ...
    public DefaultMQProducer determineRocketMQProducer(boolean isSequence,
        ...
        if (isSequence) {
            ...
            final int index = Assignments.getInstance().assign(key, producerSize);
        ...
        if (isNull(producer)) {
            throw new DataConnectorException(String.format("Could not get producer by channel: %s, key: %s",
                    channelId, key));
        }
        if (log.isDebugEnabled()) {
    ...
    public void start() {
        safeList(producers).forEach(producer -> {
            try {
                ...
                throw new DataConnectorException(String.format("Failed to start RocketMQ producer of %s",
