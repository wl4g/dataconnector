...
public class AverageShardingStrategyTests {
    ...
    public void testShardingItems() {
        List<IDataConnectorCoordinator.ServerInstance> instances = new ArrayList<>();
        instances.add(IDataConnectorCoordinator.ServerInstance.builder().instanceId("i101001").build());
        instances.add(IDataConnectorCoordinator.ServerInstance.builder().instanceId("i101002").build());
        instances.add(IDataConnectorCoordinator.ServerInstance.builder().instanceId("i101003").build());

        IShardingStrategy strategy = new AverageShardingStrategy();
        Map<IDataConnectorCoordinator.ServerInstance, List<Integer>> sharding = strategy.getShardingItem(30, instances);
