...
public class AverageShardingStrategyTests {
    ...
    public void testShardingItems() {
        List<IStreamConnectCoordinator.ServerInstance> instances = new ArrayList<>();
        instances.add(IStreamConnectCoordinator.ServerInstance.builder().instanceId("i101001").build());
        instances.add(IStreamConnectCoordinator.ServerInstance.builder().instanceId("i101002").build());
        instances.add(IStreamConnectCoordinator.ServerInstance.builder().instanceId("i101003").build());

        IShardingStrategy strategy = new AverageShardingStrategy();
        Map<IStreamConnectCoordinator.ServerInstance, List<Integer>> sharding = strategy.getShardingItem(30, instances);
