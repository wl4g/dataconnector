...
import com.wl4g.dataconnector.stream.AbstractStream.BaseStreamConfig;
...
import static com.wl4g.infra.common.collection.CollectionUtils2.*;
...
/**
 * The {@link DataConnectorEngineBootstrap}
 *
 * @author James Wong
 * @since v1.0
 **/
...
public class DataConnectorEngineBootstrap extends Collector implements ApplicationRunner, Closeable {

    private final AtomicBoolean running = new AtomicBoolean(false);
    private final Environment environment;
    private final DataConnectorConfiguration config;
    private final CachingChannelRegistry registry;
    private final Map<String, DataConnectororBootstrap> connectorRegistry;
    private final IDataConnectorCoordinator coordinator;

    public DataConnectorEngineBootstrap(@NotNull Environment environment,
    ...
    public Map<String, DataConnectororBootstrap> getConnectorBootstraps() {
        return unmodifiableMap(connectorRegistry.entrySet().stream()
                .collect(toMap(Entry::getKey,
                        e -> new DataConnectororBootstrap(e.getKey(),
    ...
    private void registerAllConnectors() {
        ...
        if (log.isInfoEnabled()) {
            final StringBuffer message = new StringBuffer("----- ::: [Begin] Registered all connectors details ::: -----%n");
                    ...
                    .forEach((connectorName, connectorBootstrap) -> {
                        ...
                        safeMap(connectorBootstrap.getSourceBootstraps())
                                .forEach((sourceName, value) -> {
                                    message.append(String.format("Registered channel source bootstrap of connector: %s, source: %s%n",
                                            connectorName, value.getStream().getDescription()));
                                });
                        // channelId -> bootstrap
                        safeMap(connectorBootstrap.getSinkBootstraps())
                                .forEach((channelId, value) -> {
                                    message.append(String.format("Registered channel sink bootstrap of connector: %s, sink: %s%n",
                                            connectorName, value.getStream().getDescription()));
                                });
                    });
            message.append("----- ::: Registered all connectors details [End] ::: -----%n");
    ...
    private void startAllConnectors() {
        ...
        connectorRegistry.values().forEach(DataConnectororBootstrap::startSources);

        log.info("Starting to all connectors sink for {}...", connectorRegistry.size());
        connectorRegistry.values().forEach(DataConnectororBootstrap::startSinks);
    }
    ...
    public DataConnectororBootstrap registerConnector(ConnectorConfig connectorConfig) {
        ...
        if (connectorRegistry.containsKey(connectorConfig.getName())) {
            throw new DataConnectorException(String.format("Already to registered channel connector: %s",
        ...
        return connectorRegistry.computeIfAbsent(connectorConfig.getName(), connectorName -> {
            ...
            final StreamContext context = new StreamContext(environment,
            ...
            return new DataConnectororBootstrap(connectorName, sourceBootstraps, sinkBootstraps);
        });
    }
    ...
    public StreamBootstrap<? extends SourceStream> createConnectorSource(
        ...
        return DataConnectorSpiFactory
                .get(SourceStreamProvider.class, BaseStreamConfig.getStreamProviderTypeName(sourceStreamConfig.getType()))
    ...
    @AllArgsConstructor
    @Getter
    public static class DataConnectororBootstrap {
