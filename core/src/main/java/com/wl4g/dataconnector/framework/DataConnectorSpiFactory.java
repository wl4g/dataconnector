...
/**
 * The {@link DataConnectorSpiFactory}
 *
 * @author James Wong
 * @since v1.0
 **/
@NoArgsConstructor(access = AccessLevel.PRIVATE)
public class DataConnectorSpiFactory {

    private final static Map<Class<? extends IDataConnectorSpi>, Map<String, IDataConnectorSpi>>
     ...
     * @return {@link IDataConnectorSpi}
     */
    @SuppressWarnings("unchecked")
    public static <T extends IDataConnectorSpi> T get(@NotNull Class<T> spiClass,
    ...
    public static IDataConnectorSpi obtain(Class<? extends IDataConnectorSpi> spiClass,
                                           String type) {
        return safeList(load(spiClass))
                .stream()
                .filter(f -> StringUtils.equals(type, f.getType()))
                .findFirst()
                .orElseThrow(() -> new DataConnectorException(String.format("The custom stream connect SPI " +
    ...
    public static <T extends IDataConnectorSpi> List<T> load(Class<T> spiClass) {
