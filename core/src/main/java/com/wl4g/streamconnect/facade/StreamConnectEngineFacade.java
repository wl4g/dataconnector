...
public class StreamConnectEngineFacade {
    ...
    public Map<String, StreamConnectEngineBootstrap.StreamConnectorBootstrap> getConnectorRegistry() {
        return engineBootstrap.getConnectorRegistry();
    }

    public StreamConnectEngineBootstrap.StreamConnectorBootstrap registerConnector(ConnectorConfig connectorConfig) {
    ...
    public @NotNull Map<String, StreamConnectEngineBootstrap.ContainerTaskStatus> statusSources(@NotBlank String connectorName,
                                                                                                String... sourceNames) {
        return getRequiredConnectorBootstrap(connectorName).statusSources(sourceNames);
    }

    public @NotNull Map<String, StreamConnectEngineBootstrap.ContainerTaskStatus> statusSinks(@NotBlank String connectorName,
                                                                                              String... channelIds) {
    ...
    private StreamConnectEngineBootstrap.StreamConnectorBootstrap getRequiredConnectorBootstrap(
            @NotBlank String connectorName) {
        Assert2.hasTextOf(connectorName, "connectorName");
        final StreamConnectEngineBootstrap.StreamConnectorBootstrap connector = getConnectorRegistry().get(connectorName);
