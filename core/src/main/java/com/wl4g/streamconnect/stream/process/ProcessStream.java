...
public class ProcessStream extends AbstractStream {
    ...
    public Queue<ICheckpoint.WritePointResult> process(List<? extends MessageRecord<String, Object>> records) {
        return doParallelProcessAndSavepoint(records);
    }

    private Queue<ICheckpoint.WritePointResult> doParallelProcessAndSavepoint(
        ...
        final Queue<ICheckpoint.WritePointResult> writePointResults = new ConcurrentLinkedQueue<>();
        while (!processResults.isEmpty()) {
            ...
            if (pr.getFuture().isDone()) {
                ...
                if (nonNull(pm) && pm.isMatched()) {
                    ...
                    final ICheckpoint.PointWriter pointWriter = obtainChannelPointWriter(pr.getRecord().getChannel());
    ...
    private Callable<ProcessMetadata> buildProcessTask(ChannelRecord cr,
        ...
        return () -> {
            final ComplexProcessChain.ComplexProcessResult result = chain.process(cr.getChannel(), cr.getRecord());
    ...
    public ICheckpoint.PointWriter obtainChannelPointWriter(@NotNull ChannelInfo channel) {
    ...
    public void flushWritePoints(@NotNull Collection<ICheckpoint.WritePointResult> writePointResults) {
