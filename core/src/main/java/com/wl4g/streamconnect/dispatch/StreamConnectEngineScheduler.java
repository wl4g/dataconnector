...
import com.wl4g.streamconnect.config.KafkaConsumerBuilder;
...
/**
 * The {@link StreamConnectEngineScheduler}
 *
 * @author James Wong
 * @since v1.0
 **/
...
public class StreamConnectEngineScheduler implements ApplicationRunner, Closeable {
    ...
    public StreamConnectEngineScheduler(@NotNull StreamConnectConfiguration config,
                                        @NotNull CheckpointTopicManager topicManager,
                                        @NotNull StreamConnectEngineCustomizer customizer,
                ...
                .forEach((pipelineName, pipelineBootstrap) -> {
                    ...
                    safeMap(pipelineBootstrap.getFilterBootstraps())
                            .forEach((sourceName, value) -> log.info("Registered filter subscribe bootstrap of pipeline: {}, source: {}, topic: {}, groupId: {}",
                                    pipelineName, value.getDispatcher().getSubscribeSourceProps().getName(),
    ...
    public SubscribePipelineBootstrap registerPipeline(PipelineConfig pipelineConfig) {
        ...
        return pipelineRegistry.computeIfAbsent(pipelineConfig.getName(), pipelineName -> {
            ...
            final IProcessSink sink = pipelineConfig.getSink();
    ...
    public SubscribeContainerBootstrap<ProcessBatchMessageDispatcher> registerPipelineFilter(
        ...
        final ProcessBatchMessageDispatcher dispatcher = new ProcessBatchMessageDispatcher(
                config,
                pipelineConfig,
    ...
    public SubscribeContainerBootstrap<SinkBatchMessageDispatcher> registerPipelineSink(
        ...
        final String sinkGroupId = customizer.generateSinkGroupId(pipelineConfig.getName(), sinkProps, subscriber.getId());
        ...
        final SinkBatchMessageDispatcher dispatcher = new SinkBatchMessageDispatcher(
                ...
                sinkGroupId,
                subscriber,
                pipelineConfig.getSink());

        return new SubscribeContainerBootstrap<>(dispatcher,
                new KafkaConsumerBuilder(sinkProps.getConsumerProps())
                        .buildSubscriber(Pattern.compile(sinkFromTopic), sinkGroupId,
                                sinkProps.getParallelism(), dispatcher));
    }
    ...
    public static class SubscribePipelineBootstrap {
        ...
        private final Map<String, SubscribeContainerBootstrap<ProcessBatchMessageDispatcher>> filterBootstraps; // source=>bootstrap
        ...
        public Map<String, Boolean> startFilters(String... sourceNames) {
            return safeMap(filterBootstraps).entrySet().stream()
                    ...
                    .collect(Collectors.toMap(
                            entry -> {
                                try {
                                    final SubscribeContainerBootstrap<ProcessBatchMessageDispatcher> bootstrap = entry.getValue();

                                    log.info("Starting filter subscribe bootstrap of pipeline: {}, source: {}, topic: {}, groupId: {}",
                                            name, bootstrap.getDispatcher().getSubscribeSourceProps().getName(),
                                    ...
                                    log.error("Failed to start filter subscribe bootstrap of pipeline: {}, source: {}, topic: {}, groupId: {}",
                                            name, entry.getValue().getDispatcher().getSubscribeSourceProps().getName(),
        ...
        public Map<String, Boolean> stopFilters(long perFilterTimeout, String... sourceNames) {
            ...
            final Map<String, Boolean> result = safeMap(filterBootstraps).entrySet().stream()
                    .filter(e -> isEmptyArray(sourceNames) || equalsAny(e.getKey(), sourceNames))
                    .collect(Collectors.toMap(
                            //entry -> entry.getValue().getDispatcher().getSubscribeSourceProps().getName(),
                            Entry::getKey,
                            entry -> {
                                final SubscribeContainerBootstrap<ProcessBatchMessageDispatcher> bootstrap = entry.getValue();
                                final String sourceName = bootstrap.getDispatcher().getSubscribeSourceProps().getName();
                                try {
