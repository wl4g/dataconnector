...
public class StreamConnectConfiguration {
    ...
    private final IStreamConnectCoordinator.CoordinatorProvider coordinatorProvider;
    ...
    public StreamConnectConfiguration validate() {
        ...
        safeList(properties.getConnectors())
                .stream()
                .map(StreamConnectProperties.ConnectorProperties::getCheckpoint)
                .collect(toMap(e -> e, e -> 1, Integer::sum))
                .entrySet()
                .stream()
                .filter(e -> e.getValue() > 1)
    ...
    private DefinitionsConfig parseDefinitionsConfig(@NotNull StreamConnectProperties properties) {
        final StreamConnectProperties.DefinitionProperties definitionProps = properties.getDefinitions();
        ...
        // Init to checkpoint providers.
        final Map<String, IStreamConnectCoordinator.CoordinatorProvider> coordinatorProviderMap =
                safeList(definitionProps.getCoordinators())
                        .stream()
                        .collect(toMap(IStreamConnectCoordinator.CoordinatorProvider::getName, e -> e));
        definitions.setCoordinatorProviderMap(coordinatorProviderMap);

        return definitions;
    }

    private ConnectorConfig parseConnectorConfig(@NotNull StreamConnectProperties.ConnectorProperties connectorProps) {
    ...
    public static class DefinitionsConfig {
        ...
        private @NotEmpty Map<String, IStreamConnectCoordinator.CoordinatorProvider> coordinatorProviderMap;
    ...
    public static class ConnectorConfig {
        ...
        private @NotNull ProcessStream.ProcessStreamConfig processConfig;
