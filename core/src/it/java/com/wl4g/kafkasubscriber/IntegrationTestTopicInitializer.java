...
public class IntegrationTestTopicInitializer {
        ...
        try (AdminClient adminClient = AdminClient.create(Collections.singletonMap(ProducerConfig.BOOTSTRAP_SERVERS_CONFIG, testBrokerServers));) {
            try {
                ...
                if (reason instanceof UnknownTopicOrPartitionException) {
                    System.err.println("Not found test topic: " + testInputTopic);
                } else {
                    ex.printStackTrace();
                }
            }
            try {
                ...
                if (ex instanceof TopicExistsException) {
                    System.err.println("Already exists test topic: " + testInputTopic);
                } else {
                    ex.printStackTrace();
                }
            }
    ...
    private void startProduceTestTopic() {
        ...
        final Producer<String, String> producer = new KafkaProducerBuilder(configProps).buildProducer();

        final AtomicLong counter = new AtomicLong(0);
        final int total = 100, recordsPerExecutor = 10;
        final CountDownLatch latch = new CountDownLatch(total / recordsPerExecutor);
