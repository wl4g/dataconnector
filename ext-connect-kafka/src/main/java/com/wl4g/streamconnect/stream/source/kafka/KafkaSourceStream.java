...
public class KafkaSourceStream extends SourceStream
    ...
    private void process(List<? extends MessageRecord<String, Object>> records,
                         Acknowledgment ack) {
        final Queue<ICheckpoint.WritePointResult> writePointResults = getProcessStream().process(records);
        ...
        if (getConnectorConfig().getQos().supportRetry(getConnectorConfig())) {
            ...
            final Set<ICheckpoint.WritePointResult> completedResults = new HashSet<>(writePointResults.size());
            for (int i = 0; !writePointResults.isEmpty(); i++) {
                final ICheckpoint.WritePointResult wpr = writePointResults.poll();
                        ...
                        if (ex instanceof ExecutionException) {
                            getConnectorConfig().getQos().retryIfFail(getConnectorConfig(),
                                    wpr.getRetryTimes(), () -> {
                                        ...
                                        final ICheckpoint.PointWriter pointWriter = getProcessStream().obtainChannelPointWriter(wpr.getRecord().getChannel());
