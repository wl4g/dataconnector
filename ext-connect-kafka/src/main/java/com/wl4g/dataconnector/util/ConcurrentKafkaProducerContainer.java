...
public class ConcurrentKafkaProducerContainer implements Closeable {
    ...
    public ConcurrentKafkaProducerContainer(@NotBlank String name,
                                            ...
                                            @Nullable String transactionIdPrefix,
    ...
    public Producer<String, Object> obtainFirstProducer() {
        if (producerFactories.isEmpty()) {
            throw new DataConnectorException("The producer factory instances is empty " +
    ...
    public Producer<String, Object> obtainDetermineProducer(@NotNull String key,
        ...
        if (isSequence) {
            ...
            final int index = Assignments.getInstance().assign(key, producerFactories.size());
        ...
        if (isNull(producerFactory)) {
            throw new DataConnectorException(String.format("%s :: Could not get producer factory for key: %s", name, key));
        }
        if (log.isDebugEnabled()) {
    ...
    public synchronized void close(@NotNull Duration timeout) {
