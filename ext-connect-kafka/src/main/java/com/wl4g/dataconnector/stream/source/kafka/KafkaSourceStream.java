...
import com.wl4g.dataconnector.meter.DataConnectorMeter.MetricsTag;
...
import com.wl4g.dataconnector.util.KafkaConsumerBuilder;
import com.wl4g.dataconnector.util.KafkaConsumerBuilder.ObjectNodeDeserializer;
...
import static com.wl4g.dataconnector.stream.AbstractStream.BaseStreamConfig.getStreamProviderTypeName;
import static com.wl4g.dataconnector.util.ConcurrentKafkaProducerContainer.buildDefaultAcknowledgedProducerContainer;
...
public class KafkaSourceStream extends SourceStream
    ...
    public void onMessage(@NotNull List<ConsumerRecord<String, Object>> records,
        ...
        try {
            ...
            handleMessages(KafkaMessageRecord.from(records), ack);
        } catch (Exception ex) {
    ...
    private void handleMessages(List<? extends MessageRecord<String, Object>> records,
                                Acknowledgment ack) {
        ...
        final Queue<WritePointResult> writeResults = getDispatchStream().dispatch(records);
        ...
        waitForDispatched(writeResults, ack);
    ...
    private void waitForDispatched(Queue<WritePointResult> writeResults, Acknowledgment ack) {
        ...
        if (getConnectorConfig().getQos().supportRetry(getConnectorConfig())) {
            final int initSize = writeResults.size();
            final Set<WritePointResult> completedResults = new HashSet<>(writeResults.size());
            for (int i = 0; !writeResults.isEmpty(); i++) {
                final WritePointResult wpr = writeResults.poll();
                        ...
                        if (ex instanceof ExecutionException) {
                            getConnectorConfig().getQos().retryIfFail(getConnectorConfig(),
                                    wpr.getRetryTimes(), () -> {
                                        ...
                                        @SuppressWarnings("all") final PointWriter pointWriter = getDispatchStream()
                                                .obtainChannelPointWriter(wpr.getChannel());
                                        writeResults.offer(pointWriter.writeAsync(getConnectorConfig(),
                                                wpr.getChannel(), wpr.getRecord(), wpr.getRetryTimes() + 1));
                                    });
                        }
                    }
                }
                ...
                if (completedSize == initSize || i % max(completedSize / 4, 128) == 0) {
                    getDispatchStream().flushWritten(completedResults);
                }
            ...
            if (getConnectorConfig().getQos().supportPreferAcknowledge(getConnectorConfig())) {
                ...
                final long acknowledgeTimingBegin = nanoTime();
                try {
                    if (log.isDebugEnabled()) {
                    ...
                    getEventPublisher().publishEvent(TimingMeterEvent.of(
                            ...
                            DataConnectorMeter.DEFAULT_PERCENTILES,
                            Duration.ofNanos(nanoTime() - acknowledgeTimingBegin),
                            getBasedMeterTags(),
                            MetricsTag.ACK_KIND,
        ...
        // are low and data losses are allowed and commit it directly.
            ...
            try {
                ...
                postAcknowledgeCountMeter(MetricsName.acknowledge_success, writeResults);
            } catch (Exception ex) {
                log.error(String.format("%s :: %s :: Failed to sent force acknowledge for %s",
                        getConnectorConfig().getName(), sourceStreamConfig.getGroupId(), ack), ex);
                postAcknowledgeCountMeter(MetricsName.acknowledge_failure, writeResults);
            } finally {
                getEventPublisher().publishEvent(TimingMeterEvent.of(
                        MetricsName.acknowledge_time,
                        DataConnectorMeter.DEFAULT_PERCENTILES,
    ...
    private void postAcknowledgeCountMeter(MetricsName metrics,
                                           Collection<ICheckpoint.WritePointResult> writePointResults) {
        writePointResults.stream()
                .map(sr -> {
                    final MessageRecord<String, Object> r = sr.getRecord();
     ...
     */
    private void preferAutoAcknowledge(Set<ICheckpoint.WritePointResult> completedResults) {
        // grouping and sorting.
        final Map<TopicPartition, List<KafkaMessageRecord<String, Object>>> partitionRecords =
                new HashMap<>(completedResults.size());
        for (ICheckpoint.WritePointResult writePointResult : completedResults) {
            final KafkaMessageRecord<String, Object> _record = (KafkaMessageRecord<String, Object>) writePointResult.getRecord();
        ...
        try {
            ...
            final String errmsg = String.format("%s :: Failed to prefer acknowledge offsets to transaction. - : %s",
                    sourceStreamConfig.getGroupId(), partitionOffsets);
            log.error(errmsg, ex);
            throw new DataConnectorException(errmsg, ex);
        } finally {
            getEventPublisher().publishEvent(TimingMeterEvent.of(
                    MetricsName.acknowledge_time,
                    DataConnectorMeter.DEFAULT_PERCENTILES,
    ...
    public static class KafkaSourceStreamConfig extends SourceStreamConfig {
        ...
        public static final Map<String, Object> DEFAULT_CONSUMER_PROPS = unmodifiableMap(new HashMap<String, Object>() {
            {
